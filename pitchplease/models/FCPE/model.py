...
from pitchplease.models.FCPE.modules import ConformerNaiveEncoder
...
class CFNaiveMelPE(nn.Module):
    ...
    def __init__(
        ...
        self.gaussian_blurred_cent_mask_b = (1200.0 * torch.log2(torch.Tensor([self.f0_max / 10.0])))[0].detach()
    ...
    @torch.no_grad()
    def latent2cents_decoder(self, y: torch.Tensor, threshold: float = 0.05, mask: bool = True) -> torch.Tensor:
        """
    ...
    @torch.no_grad()
    def latent2cents_local_decoder(self, y: torch.Tensor, threshold: float = 0.05, mask: bool = True) -> torch.Tensor:
        """
    ...
    def f0_to_cent(self, f0: torch.Tensor) -> torch.Tensor:
        ...
        """
        cent = 1200.0 * torch.log2(f0 / 10.0)
        return cent  # (B, T, 1)
